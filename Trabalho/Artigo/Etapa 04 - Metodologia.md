...
O fluxo de vida desejado (**To Be**) e o esboço do protótipo de baixa fidelidade já foram elaborados a partir dos requisitos preliminares. O fluxo atual (**As Is**) é uma hipótese analítica derivada da problemática e do referencial, usada para contrastar o problema com a solução proposta; não descreve empiricamente a rotina de uma agência específica. A representação do processo futuro adota Business Process Model and Notation (BPMN) 2.0, distinguindo Cliente e Agência como participantes e Atendimento/Gestor de conta e Gestor de tráfego como responsabilidades internas (OMG, 2011). O refinamento documentado separa transferência de trabalho entre perfis da navegação da sessão, distingue correção em validação de reabertura após conclusão, permite conclusão direta apenas para tipos que dispensam aprovação e registra a origem do aguardo para que a retomada ocorra na fase correta. Também define a classificação do SLA após a abertura, sem converter urgência informada em prioridade automática.
...
As tarefas serão distribuídas conforme o perfil do participante, sem conceder permissões de outro perfil na mesma sessão. Clientes abrirão, complementarão e aprovarão/corrigirão ou reabrirão; participantes de Atendimento classificarão, atribuirão e retomarão; participantes de tráfego registrarão a execução; e o roteiro poderá usar sessões fictícias separadas para mostrar as transferências entre papéis. O conjunto de tarefas avaliará abertura, busca/filtro, triagem, execução, complemento, validação e reabertura. Serão observados obstáculos, dúvidas, erros, necessidade de ajuda e tempo de realização.
